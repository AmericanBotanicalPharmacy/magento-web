...
class Fishpig_Wordpress_Block_Videofavorite extends Fishpig_Wordpress_Block_Post_List_Wrapper_Abstract
{
	/**
	...
	protected function _beforeToHtml()
	{
		...
		if (!$this->getTemplate()) {
			$this->setTemplate('wordpress/video-favorite.phtml');
		}
	...
	{
		return $this->_getData('number') ? $this->_getData('number') : 100;
	}
	 ...
	 */
	public function _getPostCollection()
	{
          $args =  array(1507, 1462, 1504, 1429, 1506, 1501, 1500, 1498, 1418, 1440, 1495, 1491, 1474, 1452, 1431, 1425, 1414) ; 
         
          if (is_null($this->_collection)) {
			$collection = Mage::getResourceModel('wordpress/post_collection')
				//->setOrderByPostId($args)
//				->setPageSize($this->getNumber())
//				->setCurPage(1);
				->addPostIdFilter($args);
			if (Mage::getSingleton('customer/session')->isLoggedIn() && Mage::helper('wordpress')->isAddonInstalled('CS')) {
				...
				$collection->addStatusFilter('publish');
                            ...
                            $collection->addPostTypeFilter('video');
                          
                            $collection->getSelect()->order(new Zend_Db_Expr('FIELD(`main_table`.`ID`, ' . implode(',', $args).')'));
