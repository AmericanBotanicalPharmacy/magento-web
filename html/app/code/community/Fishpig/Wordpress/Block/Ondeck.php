...
class Fishpig_Wordpress_Block_Ondeck extends Fishpig_Wordpress_Block_Post_List_Wrapper_Abstract
{
	/**
	...
	protected function _beforeToHtml()
	{
		...
		if (!$this->getTemplate()) {
			$this->setTemplate('wordpress/ondeck.phtml');
		}
...
//				->setCurPage(1);
			...
			if (Mage::getSingleton('customer/session')->isLoggedIn() && Mage::helper('wordpress')->isAddonInstalled('CS')) {
				...
				$collection->addStatusFilter('private');
			}
	 
                            $collection->addPostTypeFilter('video');
                            $collection->addPostTypeFilter('commentary');
