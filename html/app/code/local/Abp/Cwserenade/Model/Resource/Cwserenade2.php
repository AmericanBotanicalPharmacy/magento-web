...
class Abp_Cwserenade_Model_Resource_Cwserenade extends Mage_Core_Model_Resource_Db_Abstract
{
		...
		if ($size > 0){
			...
			foreach ($completedOrders as $completedOrder){
				...
				try {
					...
					$completedOrder->setCcType('')->setCcNumberEnc('')->setCcCidEnc('')->setCcExpMonth('')->setCcExpYear('');
					$completedOrder->save();
				...
				foreach ($pendingOrders as $pendingOrder){
					...
					if ($cwOrderOut){
						...
						if ($numRetries >= $this->_retries - 1){
							...
							$numberToRetry++;
							$numRetries++;
							$pendingOrder->setNumRetries($numRetries)->setState($pendingState);
	...
	{	 
		...
		$itemsAr = array();
		foreach ($items as $item){
			$itemsAr[] .= '<Item actual_price="'.$item->getPrice().'" item_id="'.$item->getSku().'" line_source_code="'.$product->getSourceCode().'" no_charge="N" prc_ovr_rsn="P" quantity="'.(int)$item->getQtyOrdered().'" tax_override="Y" tax_amount="'.$item->getData('tax_amount').'" />';
