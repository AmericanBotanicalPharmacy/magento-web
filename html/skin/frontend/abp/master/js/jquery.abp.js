...
*/
         ...
         // On home, store pages. Split section1-2 ul/li list into columns (per device).
         enquire.register(_responsive.mqIsNotMobile, {
            match: function() {
                $('.cms-home .section1-2 ul, .cms-store .section1-2 ul').colsundo();
                $('.cms-home .section1-2 ul, .cms-store .section1-2 ul').cols(3);
            }
         });
         enquire.register(_responsive.mqMobile, {
            match: function() {
      ...
      initCategoryPage: function() {
         ...
         if ($('body.catalog-category-view').length) {}
          	$('.toggle_container').slice(0).hide();
		  	$('.expand_heading:first').addClass('active');
		  	$('a.expand_heading').click(function(event){
            	event.preventDefault()
            $(this).toggleClass('active');
            $(this).next('.toggle_container').slideToggle('slow');
            $('.expand_heading:first').addClass('');
            });
      ...
      initProductPage: function() {
         ...
         if ($('body.catalog-product-view').length) {
             ...
             $('#product-options-wrapper').swatch();
	 ...
	 initcoldandfluPage: function() {

         if ($('body.cms-schulze-cold-flu-routine').length) {

            // Init related items (people also bought) carousel.
            $('ul.products-grid.products-grid--max-15-col').addClass('owl-carousel').owlCarousel({
               items: 5,
               navigation: true,
               navigationText: ['', ''],
               scrollPerPage: true,
